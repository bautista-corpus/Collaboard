...
canvas.on('mouse:down', function(o) {
    if (freeDrawing) {
        ...
        if(isRectActive) {
            ...
            textarea = new fabric.Text("texto", {
                fontSize: 30,
                fill : 'black',
                type : 'text',
                selectable : false,
...
canvas.on('mouse:move', function(o) {
    if (isDown && freeDrawing) {
        ...
        if(isRectActive) {
            ...
            if(origX>pointer.x){
                textarea.set({ left: Math.abs(pointer.x) });
            }
            if(origY>pointer.y){
                textarea.set({ top: Math.abs(pointer.y) });
            }

            textarea.set({ width: Math.abs(origX - pointer.x) });
            textarea.set({ height: Math.abs(origY - pointer.y) });
        }
        canvas.renderAll();
    }
});
